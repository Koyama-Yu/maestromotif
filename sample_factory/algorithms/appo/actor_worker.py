...
class ActorState:
    ...
    def record_env_step(self, reward, done, info, rollout_step):
        ...
        self.curr_traj_buffer['step'][rollout_step] = info['step']
    ...
    def episodic_stats(self, last_episode_true_reward, last_episode_extra_stats, info):
        ...
        stats['timestep'] = info.get('timestep', 0)
        stats['gold'] = info.get('gold', 0)
        stats['xlvl'] = info.get('xlvl', 0)
        stats['scout'] = info.get('scout', 0)
        stats['num_buc'] = info.get('num_buc', 0)
        stats['num_sold'] = info.get('num_sold', 0)
        stats['num_price_id'] = info.get('num_price_id', 0)
        stats['num_sell'] = info.get('num_sell', 0)
        stats['altar_seen'] = info.get('altar_seen', 0)
        stats['shop_seen'] = info.get('shop_seen', 0)
        stats['branch_id'] = info.get('branch_id', 0)
        stats['reached'] = info.get('reached', 0)
...
class VectorEnvRunner:
    ...
    def __init__(self, cfg, num_envs, worker_idx, split_idx, num_agents, shared_buffers, pbt_reward_shaping):
        ...
        self.num_agents = num_agents  # queried from env, typically single agent
    ...
    def _process_env_step(self, new_obs, rewards, dones, infos, env_i):
        ...
        for agent_i in range(self.num_agents):
            ...
            # this would reset the rnn states such that new_obs starts from the right states
