...
class LearnerWorker:
    def __init__(
        ...
        self.missed_bucs = deque([], maxlen=50)
        ...
        for epoch in range(self.cfg.ppo_epochs):
            ...
            for batch_num in range(len(minibatches)):
                ...
                with timing.add_time('bptt_initial'):
                    if self.cfg.use_rnn:
                        # Don't reset rnn states between option switches
                        dones = mb.dones_cpu
                        head_output_seq, rnn_states, inverted_select_inds = build_rnn_inputs(
                            head_outputs, dones, mb.rnn_states, recurrence,
                        )
                    else:
                ...
                with timing.add_time('tail'):
                    ...
                    result = self.actor_critic.forward_tail(core_outputs, mb.obs, with_action_distribution=True)
                ...
                with torch.no_grad():  # these computations are not the part of the computation graph
                    ...
                    if self.cfg.llm_reward > 0.:
                        ...
                        for o, (reward_model, rew_eps) in enumerate(zip(self.reward_models, self.rew_eps_thresholds)):

                            mask = (mb.obs['option'] == o).squeeze().cpu()

                            if mask.any():
                                # Don't normalize 'obs' since normalization happens in-place when calling self.actor_critic.forward_head
                                r_head_outputs = reward_model.forward_head(mb.obs, normalize=False) 
                                r_core_outputs, _ = reward_model.forward_core(r_head_outputs, torch.zeros_like(rnn_states))
                                llm_reward = reward_model.reward_fn(r_core_outputs)

                                _llm_rewards_cpu = llm_reward.detach().cpu().squeeze()

                                if self.cfg.rew_norm:
                                    _llm_rewards_cpu = (_llm_rewards_cpu - reward_model.mean) / (reward_model.var)**(1/2)
                                _llm_rewards_cpu = (_llm_rewards_cpu > rew_eps) * _llm_rewards_cpu

                                llm_rewards_cpu[mask] = _llm_rewards_cpu[mask]
                    ...
                    if self.cfg.with_vtrace:
                        ...
                        rewards_cpu = (
                            self.cfg.extrinsic_reward * mb.rewards_cpu
                ...
                with timing.add_time('update'):
                    ...
                    if 'none' in self.cfg.eval_target:
                        loss.backward()
            ...
            new_epoch_actor_loss = np.mean(epoch_actor_losses) if len(epoch_actor_losses) else 0.
    ...
    def init_model(self, timing):
        ...
        if self.cfg.llm_reward > 0.:
            log.info("Loading reward models...")
            reward_dirs = ast.literal_eval(self.cfg.reward_dir)
            eps_threshold_quantiles = ast.literal_eval(self.cfg.eps_threshold_quantile)
            self.reward_models = []
            self.rew_eps_thresholds = []

            for i, (reward_dir, eps_threshold_quantile) in enumerate(zip(reward_dirs, eps_threshold_quantiles)):
                checkpoints = self.get_checkpoints(join(reward_dir, f'checkpoint_p0'))
                ...
                for key in list(checkpoint_dict['model'].keys()):
                    if 'action' in key or 'critic' in key:
                        checkpoint_dict['model'].pop(key)

                self.reward_models.append(create_reward_model(self.cfg, self.obs_space, self.action_space, timing=timing))
                self.reward_models[i].load_state_dict(checkpoint_dict['model'])
                self.reward_models[i].add_mean_var(checkpoint_dict['reward_mean'].item(), checkpoint_dict['reward_var'].item())
                self.reward_models[i].model_to_device(self.device)
                self.reward_models[i].share_memory()
                if eps_threshold_quantile != 0.0:
                    try:
                        reward_csv_file = f'{reward_dir}/reward_metrics/train_norm_quantiles.csv'
                    ...
                    quantile_index = np.where(np.array(reward_quantile_info[0]).astype(float) == eps_threshold_quantile)[0][0]
                    quant = float(reward_quantile_info[-1][quantile_index])
                    if i  == 2 and '99' in self.cfg.experiment:
                        quant = 2.10
                    self.rew_eps_thresholds.append(quant)
                else:
                    self.rew_eps_thresholds.append(-1000)
